...
sitespeed.io --preScript $preScript $URL --browsertime.pageCompleteCheck "var element = document.querySelector(\"$endSelector\"); if (element == null) return false; else return element.textContent.trim() == \"$endSelectorValue\";" --outputFolder performance-report/bionexo/$version/cotacao-$(date +%Y-%m-%d-%H-%M-%S) --html.showAllWaterfallSummary -n 3
